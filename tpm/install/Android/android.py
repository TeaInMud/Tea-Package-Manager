...
TEA_PATH = os.path.join(TERMUX_BIN, "tea")

# Inject the payload architecture directly
...
def download_custom_package(name):
    """Helper function to download a package from the cloud to the system path."""
    ...
    url = PACKAGES[name]
    try:
        bin_dir = "/data/data/com.termux/files/usr/bin" if os.path.exists("/data/data/com.termux") else "/usr/local/bin"
    ...
    # NEW: Handle custom package updating
        ...
        if target == "all":
            ...
            for pkg_name in PACKAGES:
                local_file = os.path.join(bin_dir, pkg_name)
                # If the script file exists locally, run an upgrade download loop on it
                if os.path.exists(local_file):
                    print(f"Updating {pkg_name}...")
                    if download_custom_package(pkg_name):
                        updated_count += 1
...
if not os.path.exists("/data/data/com.termux"):
    print("Error: Environment is not Termux.")
    sys.exit(1)
...
print("[OK] Tea has been successfully linked to Termux global bin!")
